export function logShopifyQueryCost(extensions) {
  if (!extensions) {
    console.warn("No cost data found in the response");
    return;
  }
  ...
  if (!throttleStatus) {
    console.warn("No throttle status found in the response");
    return;
  }

  const report =
    `Shopify Query Cost Report:\n` +
    `Requested Query Cost: ${requestedQueryCost}\n` +
    `Actual Query Cost: ${actualQueryCost}\n` +
    `Throttle Status: ${JSON.stringify(throttleStatus, null, 2)}`;

  console.log(report);
}

export function removeMyShopifyDomain(shop) {
