...
import { ProductUtils } from "../products/productUtils.js";
import { getMinimalShopifyMoves } from "@acme/core";
export class BulkOperationWebhookHandler {
  ...
  const bulkOperationID = retrieveAdminLegacyResourceID(admin_graphql_api_id, "BulkOperation");
  const status = rawStatus.toUpperCase();
  const errorCode = rawErrorCode?.toUpperCase();

  if (errorCode) {
    await this.bulkOperationService.updateBulkOperation({
      id: bulkOperationID, data: { status, error_code: errorCode },
    });
  }

  const { bulkOperation } = await this.bulkOperationGrpahql.fetchBulkOperationStatus(admin_graphql_api_id);
  ...
  const merchant = await this.merchantService.getMerchant({ shop: this.shop, useCache: false });
  const { continueSellingAsOOS, excludePushDown, excludePushDownTags } = merchant;
  ...
  const minimalMoves = getMinimalShopifyMoves(overrideSortingArray, defaultOrder.map(id => parentMap.get(id)));

  // Build info quickly with a Map (avoid O(n²) .find)
  const byLegacyId = new Map(overrideSortingArray.map(p => [p.legacyResourceId, p]));
